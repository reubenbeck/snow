...
def create_plot(dict_of_snow_masses):
    ...
    fig = plt.figure(figsize=(100,85), edgecolor="Blue")
    ...
    ax.set_xlabel('Year-Month', fontsize=60)
...
def snow_time_series_monthly(start_year, end_year, start_month, end_month, region):
    """
    Creates a time_series plot of the March Yearly snow masses and includes a 5 year rolling average.
    ...
    for year in range(start_year, end_year + 1):
        for month in range(start_month, end_month):
            ...
            filenames = Data_from_range(str(year) + '-' + str(month) + '-1', str(year) + '-' + str(month) +' -12',path)
            ...
            try:
                ...
                march_yearly_snow_masses.setdefault('year-month', []).append(str(year)+ '-' + str(month))
                march_yearly_snow_masses.setdefault('snow mass', []).append(0)
                continue
